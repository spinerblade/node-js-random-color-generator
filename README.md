# Node.js Random Color Generator

Generate random colors in the command line in form of a block containing '#'

Running the program without any input will show a block of 31x9 '#' in a random color and the Hexcode of that color:

![randomColor](/screenshots/randomColor.png)

If the user inputs hue (red or orange or yellow or green or blue or purple or pink or monochrome) and luminosity (light or dark or bright), the program will show the block in a random color matching those choices, including the Hexcode of that color:

![specificColor](/screenshots/specificColor.png)

Entering incorrect input value will result into error message:

![errorMessage](/screenshots/errorMessage.png)
